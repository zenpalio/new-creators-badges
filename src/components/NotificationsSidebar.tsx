...
export type NotificationRowLinkProps = Omit<
...
export type NotificationsSidebarStatusType =
  | "success"
  | "warning"
  | "error"
  | "info"
...
const notificationsSidebarStatusColor: Record<
            ...
            >
              ...
              <div className="flex items-center justify-between px-5 py-3.5">
                <h2 className="text-base font-bold text-foreground-v2">
                  {tab === "notifications"
                    ? labels.titleNotifications
                    : labels.titleWhatsNew}
                </h2>
                <button
                  onClick={onClose}
              ...
              {systemStatusItems.length > 0 && (
                <div className="border-y border-border-v2/40">
                  {systemStatusItems.map((item, index) => (
                    ...
                    >
                      <NotificationsSidebarStatusDot
                        color={notificationsSidebarStatusColor[item.type]}
                      />
                ...
                {tab === "notifications" ? (
                  <ul className="flex flex-col">
                    {notifications.map((n) => {
                      ...
                      const LinkRoot = (NotificationLink ??
                        "a") as ElementType<NotificationRowLinkProps>
                      const rowClassName = `relative flex w-full items-center gap-3 px-5 py-3 text-left transition-colors ${
                        isInteractive
                          ? "cursor-pointer hover:bg-muted-v2/30"
                          : ""
                      ...
                      const rowInner = (
                        <>
                          ...
                          <div className="min-w-0 flex-1 text-xs leading-snug text-foreground-v2/90">
                            <span className="font-semibold text-foreground-v2">
                              {n.actor}
                            </span>{" "}
                            <span className="text-muted-v2-foreground">
                              {n.action}
                            </span>
                            {n.target && (
                              <span className="font-semibold text-foreground-v2">
                      ...
                      <li key={a.id}>
                        ...
                        >
                          <div className="flex items-center gap-2 text-[10px] font-medium">
                            <span className="rounded-full bg-primary-v2/15 px-2 py-0.5 text-primary-v2">
                              {a.tag}
                            </span>
                            <span className="text-muted-v2-foreground">
                              {a.date}
                          ...
                          <h3 className="text-xs font-semibold leading-snug text-foreground-v2">
                            {a.title}
                          </h3>
                          <p className="line-clamp-2 text-[11px] leading-snug text-muted-v2-foreground">
                            {a.description}
                          </p>
                        </button>
                      </li>
