  ...
  <a href={href} onClick={onClick} className="block shrink-0">
    <div className="group flex h-full w-[300px] shrink-0 flex-col gap-2 rounded-2xl border border-white/[0.06] bg-grey-dark-1/60 p-4 text-left transition-colors hover:border-white/10 hover:bg-grey-dark-1">
      <div className="flex items-center gap-2 text-[11px] font-medium">
        <span className="rounded-full bg-primary/15 px-2 py-0.5 text-primary">{tag}</span>
        <span className="text-grey-light-4">{date}</span>
      </div>
      <h3 className="line-clamp-2 text-sm font-semibold leading-snug text-white">{title}</h3>
      <p className="line-clamp-2 text-xs leading-snug text-grey-light-3">{description}</p>
