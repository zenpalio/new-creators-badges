...
export default function CreateYourOwnDialog({
  ...
  description = "Don't see your perfect match? Build her from scratch — looks, personality, and story.",
  benefits = DEFAULT_BENEFITS,
  ctaLabel = "Start creating — free",
}: Props) {
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="left-0 top-0 z-[120] flex h-[100dvh] w-screen max-w-none translate-x-0 translate-y-0 flex-col gap-0 overflow-hidden rounded-none border-0 bg-[#0a0a0a] p-0 text-white data-[state=closed]:zoom-out-100 data-[state=open]:zoom-in-100 sm:left-[50%] sm:top-[50%] sm:h-auto sm:max-h-[88vh] sm:w-[calc(100%-32px)] sm:max-w-[640px] sm:translate-x-[-50%] sm:translate-y-[-50%] sm:flex-row sm:rounded-2xl sm:border sm:border-white/10">
        <div className="relative aspect-square min-h-[220px] w-full shrink-0 overflow-hidden sm:aspect-auto sm:w-[42%]">
          <img src={imageUrl} alt="" className="absolute inset-0 h-full w-full object-cover" />
          <div
            className="absolute inset-x-0 bottom-0 h-24"
            style={{ background: `linear-gradient(180deg, transparent 0%, ${accent}20 100%)` }}
          />
        </div>
